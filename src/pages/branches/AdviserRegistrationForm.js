...
const STEPS = [
  { num: 1, label: 'Promoter Verify' },
  { num: 2, label: 'Personal Info' },
  { num: 3, label: 'Address & KYC' },
  { num: 4, label: 'Nominee & Bank' },
  { num: 5, label: 'Income & Review' },
];
...
const INIT = {
  promoter_adviser_id: '', promoter_name: '', promoter_rank: '', promoter_rank_id: '',
  registration_date: todayISO(),
  rank_id: '', adviser_fee: String(ADVISER_FEE), payment_mode: 'Cash',
  branch_id: '',
...
function Step1({ form, set, errors, branches, defaultBranchId }) {
  ...
  const verify = async () => {
    ...
    setBusy(true);
    try {
      const { data } = await api.post('/api/advisers/verify-promoter', { adviser_code: id });
      if (data.success) {
        const a = data.data;
        const ranks = a.allowed_ranks || [];
        const defaultRank = a.max_allowed_rank_id || (ranks.length ? ranks[ranks.length - 1].id : '');
        set(p => ({
          ...p,
          promoter_name: a.full_name || '',
          promoter_rank: a.rank_name || a.promoter_rank_display || '',
          promoter_rank_id: a.rank_id || a.promoter_rank_id || '',
          promoter_adviser_id: a.adviser_code || id,
          allowed_ranks: ranks,
          rank_id: defaultRank ? String(defaultRank) : '',
          branch_id: p.branch_id || defaultBranchId || '',
        }));
        toast.success(`Verified: ${a.full_name}`);
      }
    } catch (e) {
      toast.error(e.response?.data?.message || 'Promoter ID not found');
      set(p => ({ ...p, promoter_name: '', promoter_rank: '', allowed_ranks: [], rank_id: '' }));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div>
      ...
      <F label="Promoter Adviser ID" req err={errors.promoter_adviser_id}
        ...
        <div className="af-row-btn">
          <I
            ...
            onChange={e => set(p => ({
              ...p,
              promoter_adviser_id: e.target.value,
              promoter_name: '',
              promoter_rank: '',
              allowed_ranks: [],
              rank_id: '',
      ...
      {verified && (
        <div className="af-verified">
          ...
          <div>
            ...
            <div className="af-vrank">{form.promoter_rank} (Rank {form.promoter_rank_id})</div>
          </div>
        </div>
      )}
      ...
      <div className="af-g2">
        ...
        <F label="New Adviser Rank" req err={errors.rank_id} hint={verified ? 'Select rank for new adviser' : 'Verify promoter first'}>
          <S
            value={form.rank_id}
            onChange={e => set(p => ({ ...p, rank_id: e.target.value }))}
            opts={form.allowed_ranks.map(r => ({ value: String(r.id), label: r.label || `${r.id}. ${r.name || ''}` }))}
            ph="Verify promoter first"
            disabled={!verified || !form.allowed_ranks.length}
          />
        </F>
      </div>
      ...
      <div className="af-g2">
        <F label="Adviser Fee (₹)" req hint="Fixed registration fee">
          <I type="number" value={ADVISER_FEE} readOnly disabled />
        </F>
      ...
      {branches?.length > 1 && (
        <F label="Branch" req err={errors.branch_id}>
          <S
            value={form.branch_id}
            onChange={e => set(p => ({ ...p, branch_id: e.target.value }))}
            opts={branches.map(b => ({ value: String(b.id), label: `${b.branch_name} (${b.branch_code})` }))}
            ph="Select Branch"
          />
        </F>
      )}
      ...
      <div className="af-fee-note">
        <strong>Note:</strong> Adviser create fee is ₹{ADVISER_FEE}, deducted from branch panel limit when adviser is
        successfully created. After creating → go to <strong>Approved Adviser</strong> tab to approve and generate login credentials.
      </div>
    </div>
  );
}
...
function Step2({ form, set, errors }) {
  return (
    <div>
      ...
      <div className="af-g2">
        <F label="Salutation">
          <S value={form.salutation} onChange={e => set(p => ({ ...p, salutation: e.target.value }))} opts={SALUTATIONS} ph="Select" />
        </F>
        <F label="Full Name" req err={errors.full_name}>
          <I value={form.full_name} onChange={e => set(p => ({ ...p, full_name: e.target.value }))} placeholder="Full name as per Aadhar" />
        </F>
      </div>
        ...
        <F label="Date of Birth" req err={errors.date_of_birth}>
          <I
            ...
            onChange={e => {
              ...
              set(p => ({ ...p, date_of_birth: d, age: calcAge(d) }));
            }}
          />
        </F>
        <F label="Age" hint="Auto-calculated">
          <I value={form.age} onChange={e => set(p => ({ ...p, age: e.target.value }))} placeholder="Auto" cls={form.date_of_birth ? 'af-auto' : ''} />
        </F>
        <F label="Gender" req err={errors.gender}>
          <S value={form.gender} onChange={e => set(p => ({ ...p, gender: e.target.value }))} opts={GENDERS} ph="Select" />
        </F>
      </div>
      <div className="af-g3">
        <F label="Marital Status" req err={errors.marital_status}>
          <S value={form.marital_status} onChange={e => set(p => ({ ...p, marital_status: e.target.value }))} opts={MARITAL} ph="Select" />
        </F>
        <F label="Nationality" req>
          <S value={form.nationality} onChange={e => set(p => ({ ...p, nationality: e.target.value }))} opts={NATIONALITIES} />
        </F>
        <F label="Email">
      ...
      <div className="af-g2">
        <F label="Mobile Number" req err={errors.mobile} hint="10-digit · must be unique">
          <I
            value={form.mobile}
            onChange={e => set(p => ({ ...p, mobile: e.target.value.replace(/\D/g, '').slice(0, 10) }))}
      ...
      <F label="Aadhar Card Number" req err={errors.aadhar_number} hint="12-digit · must be unique">
        <I
          ...
          onChange={e => set(p => ({ ...p, aadhar_number: e.target.value.replace(/\D/g, '').slice(0, 12) }))}
...
function Step3({ form, set, errors }) {
  const sync = (field, val) => set(p => ({
    ...p,
    [`corr_${field}`]: val,
    ...(p.same_as_corr ? { [`perm_${field}`]: val } : {}),
  }));
    ...
    <div>
      ...
      <div className="af-addr">
        ...
        <div className="af-g3">
          ...
          <F label="Pincode" req err={errors.corr_pincode}>
            <I value={form.corr_pincode} onChange={e => sync('pincode', e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="6-digit" maxLength={6} />
          </F>
        </div>
      </div>
      ...
      <div className={`af-addr ${form.same_as_corr ? 'af-disabled' : ''}`}>
        ...
        <div className="af-g3">
          ...
          <F label="Pincode" req={!form.same_as_corr} err={errors.perm_pincode}>
            <I value={form.perm_pincode} onChange={e => set(p => ({ ...p, perm_pincode: e.target.value.replace(/\D/g, '').slice(0, 6) }))} placeholder="6-digit" maxLength={6} disabled={form.same_as_corr} />
          </F>
        </div>
      </div>
    </div>
...
function Step4({ form, set, errors }) {
  const toggleNom = e => {
    ...
    set(p => ({
      ...p,
      nominee_same_as_member: c,
      ...(c ? {
        nominee_address: p.corr_address,
        nominee_city: p.corr_city,
        nominee_state: p.corr_state,
        nominee_pincode: p.corr_pincode,
      } : {}),
    }));
  };
    ...
    <div>
      ...
      <div className={`af-addr ${form.nominee_same_as_member ? 'af-disabled' : ''}`}>
        ...
        <div className="af-g3">
          ...
          <F label="Pincode" req={!form.nominee_same_as_member} err={errors.nominee_pincode}>
            <I value={form.nominee_pincode} onChange={e => set(p => ({ ...p, nominee_pincode: e.target.value.replace(/\D/g, '').slice(0, 6) }))} placeholder="6-digit" maxLength={6} disabled={form.nominee_same_as_member} />
          </F>
        </div>
      </div>
...
const RR = ({ l, v }) => (
  <div className="af-rr"><span className="af-rrl">{l}</span><span className="af-rrv">{v || <em>—</em>}</span></div>
);
const RS = ({ title, children }) => (
  <div className="af-rsec"><div className="af-rsec-t">{title}</div>{children}</div>
);

function Step5({ form, set }) {
  const rankLabel = form.allowed_ranks.find(r => String(r.id) === String(form.rank_id))?.label
    || (form.rank_id ? `${form.rank_id}` : '');
...
const validators = {
  1: f => {
    const e = {};
    if (!f.promoter_adviser_id.trim()) e.promoter_adviser_id = 'Enter Promoter Adviser ID';
    if (!f.promoter_name) e.promoter_adviser_id = 'Please verify the Promoter ID first';
    if (!f.rank_id) e.rank_id = 'Select adviser rank';
    return e;
  },
  2: f => {
    const e = {};
  ...
  4: f => {
    ...
    else if (Number(f.nominee_age) < 0 || Number(f.nominee_age) > 120) e.nominee_age = 'Enter a valid nominee age (0–120)';
...
export default function AdviserRegistrationForm({ onSuccess, onCancel, branches = [], defaultBranchId = '' }) {
  const [step, setStep] = useState(1);
  const [form, setForm] = useState({ ...INIT, branch_id: defaultBranchId ? String(defaultBranchId) : '' });
  ...
  const submit = async () => {
    ...
    try {
      ...
      const { data } = await api.post('/api/advisers/', payload);
      toast.success(data.message || 'Adviser created! Go to Approved Adviser tab to approve.');
      if (onSuccess) onSuccess(data.data);
      setStep(1);
      setForm({ ...INIT, branch_id: defaultBranchId ? String(defaultBranchId) : '' });
    } catch (e) {
  ...
  return (
    <div className="af-form-wrap">
      ...
      <div className="af-body">
        {step === 1 && <Step1 form={form} set={setForm} errors={errors} branches={branches} defaultBranchId={defaultBranchId} />}
      ...
      <div className="af-nav">
        {step > 1
          ? <button type="button" className="af-btn-back" onClick={back} disabled={loading}>← Back</button>
          : <button type="button" className="af-btn-cancel" onClick={onCancel}>Cancel</button>}
        {step < 5
          ? <button type="button" className="af-btn-next" onClick={next}>Next →</button>
          : <button type="button" className="af-btn-submit" onClick={submit} disabled={loading}>{loading ? 'Creating…' : '✓ Create Adviser'}</button>}
      </div>
    </div>
  );
}
